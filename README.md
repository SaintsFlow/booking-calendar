...
1. [Недавние улучшения](#-недавние-улучшения)
2. [Быстрый старт](#-быстрый-старт)
3. [Архитектура](#-архитектура)
4. [Bitrix24 Integration](#-bitrix24-crm-integration)
    - [OAuth 2.0 авторизация](#oauth-20-авторизация)
    - [Синхронизация товаров](#синхронизация-товаров-с-bitrix24)
    - [CRM Pipeline](#crm-pipeline)
5. [WebSocket (Laravel Reverb)](#-websocket-integration-laravel-reverb)
6. [База данных](#️-база-данных)
7. [Frontend (Vue 3 + Inertia)](#-frontend-vue-3--inertiajs)
8. [Авторизация](#-авторизация)
9. [Административные команды](#-административные-команды)
...
### 1. Исправление проблемы с сессиями 🔐

-   **Проблема**: Частые ошибки "Сессия истекла" сразу после входа
-   **Решение**: Настроены SESSION_SECURE_COOKIE и SESSION_SAME_SITE для HTTPS
-   **Документация**: [SESSION_FIX.md](./SESSION_FIX.md)

### 2. Ограничения для администраторов 👥

-   Администраторы больше не могут быть назначены ответственными за бронирования
-   Добавлена возможность создать первого сотрудника для нового тенанта
-   Валидация на backend и frontend уровнях

### 3. Синхронизация сотрудников с Bitrix24 🔄

-   Двусторонняя синхронизация пользователей с Bitrix24
-   Команда: `php artisan bitrix24:sync-users`
-   Автоматическая синхронизация по расписанию
-   UI кнопка для ручного запуска
-   **Документация**: [BITRIX24_USERS_SYNC.md](./BITRIX24_USERS_SYNC.md)

### 4. Управление подписками 📊

-   Полноценный CRUD для подписок тенантов
-   Контроль статусов: active, suspended, cancelled, expired
-   Интерфейс для супер-админа
-   Автоматическая проверка статуса подписки при доступе

### 5. Двусторонняя синхронизация товаров/услуг ⚡

-   Экспорт услуг в Bitrix24: `php artisan bitrix24:sync-services-to`
-   Импорт товаров из Bitrix24: существующая команда
-   UI кнопки для обоих направлений синхронизации
-   **Документация**: [BITRIX24_PRODUCTS_SYNC.md](./BITRIX24_PRODUCTS_SYNC.md)

### 6. Улучшенная проверка дубликатов 🔍

-   Проверка по: контакт + время + назначенные услуги
-   Action класс `DetectDuplicateBooking` для переиспользования
-   Интеграция в создание и обновление бронирований
-   Информативные сообщения об ошибках с деталями дубликата

### 7. Отмена и восстановление бронирований ❌↩️

-   Система отмены с указанием причины
-   Восстановление отменённых броней (только будущих)
-   Фильтрация: сотрудники НИКОГДА не видят отменённые
-   Менеджеры/админы могут переключать видимость
-   **Документация**: [BOOKING_CANCELLATION.md](./BOOKING_CANCELLATION.md)
